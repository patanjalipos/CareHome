...
export class AdlChartComponent extends AppComponentBase implements OnInit {
  ...
  constructor(private optionService: OptionService,private _ConstantServices: ConstantsService,private route: ActivatedRoute,private _UtilityService: UtilityService,private _UserService:UserService,private _ADLChart:AdlChartService) {
    super();
  ...
  forkJoin(collectionNames.map((collectionName) => this.GetChartDropDownMasterList(ChartTypes.ADLChart,collectionName,1))).subscribe((responses: any[]) => {
...
if (this.preSelectedChartData.selectedChartID != null) {
  ...
  GetADLChartDetails(chartId: string) {debugger
    this._UtilityService.showSpinner();
