...
export class BehaviourChartComponent extends AppComponentBase implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const collectionNames = [
      'BehaviourPurposeOptions'
  ];

  forkJoin(collectionNames.map((collectionName) => this.GetChartDropDownMasterList(ChartTypes.BehaviourChart,collectionName,1))).subscribe((responses: any[]) => {
...
if (this.preSelectedChartData.selectedChartID != null) {
  ...
  GetBehaviourChartDetails(chartId: string) {debugger
    this._UtilityService.showSpinner();
